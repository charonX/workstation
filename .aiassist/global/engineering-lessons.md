...
## 权限拦截单一真源：拦截必须归属策略系统，严禁在工具适配层自建第二套 pre-gate（2026-09-07，2026-09-06-cli-service-connection /reflect）

- **现象**：实现初期在 `src/agent/toolAdapter.js` 的 `runBash` 中硬编码了针对清单 CLI（`claude`/`codex`/`crwl`）的未启用检查，未启用时抛出自造的 `E-CLI-NOT-ENABLED` 错误码。这导致了双重真源：权限系统由 `gotgenes` / `permissionPolicy.js` 判定并裁决，而未启用 CLI 却被执行适配层的前置门提前截杀，既绕过了统一的权限裁决审计，又引入了不必要的非标错误码（RE4-1 审查）。
- **根因**：混淆了「策略声明」与「执行守卫」的职责。未启用 CLI 属于权限规则面的项目层覆盖（未启用 → deny 规则，已启用 → 回落出厂 ask 规则），拦截应由权限系统自然产生（verdict="deny"），不属于工具适配层的业务逻辑。
- **结论**：**命令权限拦截必须严格归属于权限策略层（`permissionPolicy.js` / gotgenes），严禁在工具适配层/执行层自定义第二套 pre-gate 或非标错误码**。工具适配层只负责环境合并与超时执行；契约设计与代码实现保持单一策略真源。

## Stale-While-Revalidate 与短轮询负缓存陷阱：探针刷新保留既有有效值，瞬态失败绝不写长负缓存（2026-09-07，2026-09-06-cli-service-connection /reflect）

- **现象**：前端在进行 CLI 探针刷新（`?refresh=1`）时，若实现层将现有缓存直接重置为初始状态（如 `version: null`），会导致前端多轮短轮询（如 2s 一次）在探测中途读取到残缺值，页面闪烁；另一方面，当外部 registry（npm/pypi）网络超时或失败时，若写入了 1 小时的长负缓存，会导致前端重试轮询（2s x 3 次）全部被负缓存短路，用户恢复网络后 1 小时内无法获取最新版本（RE4-2 审查）。
- **根因**：缓存模型将「强制刷新」误实现为「立即销毁旧有效值」，且未区分「确定不存在（强负缓存）」与「网络瞬态错误（无负缓存或超短负缓存）」。
- **结论**：**异步探针与版本拉取必须遵循 Stale-While-Revalidate 模式**：主动刷新期间保留既有有效缓存直至新结果产生，保障 UI 稳定渲染；对网络超时等外部瞬态故障，只作轻量记录，绝不写入长 TTL 负缓存，确保前端重试与轮询机制能够自愈。

## 环境凭据注入纵深防御：配置阶段阻断高危注入变量，解密阶段单 key 粒度 Fail-Closed（2026-09-07，2026-09-06-cli-service-connection /reflect）

- **现象**：用户为受管 CLI 配置环境变量时，可能无意或恶意配置 `BASH_ENV`、`LD_PRELOAD`、`GIT_SSH_COMMAND`、`NODE_PATH` 等具有进程劫持能力的环境变量；而在运行期解密环境变量快照时，若因底层系统钥匙串/密钥损坏导致单个 key 解密失败，若粗暴抛出异常会导致整个 CLI 服务瘫痪，若回退原始值又会泄露密文（RE4-6 审查）。
- **根因**：环境变量配置是可控输入，直接流入子进程执行环境；跨环境解密存在钥匙串部分失效的残余风险。
- **结论**：**受管环境凭据注入必须采取纵深防御**：
  1. 在配置输入阶段（API / CLI）维护 `DANGEROUS_ENV_KEYS` 黑名单（包含 `BASH_ENV`, `LD_PRELOAD`, `DYLD_INSERT_LIBRARIES`, `NODE_OPTIONS`, `GIT_SSH_COMMAND`, `GIT_ASKPASS`, `SSH_ASKPASS`, `PERL5LIB`, `PYTHONHOME`, `NODE_PATH` 等），直接拒绝（400）高危注入变量。
  2. 在会话快照解密阶段，实行**单 key 粒度 Fail-Closed**：解密失败的单个 key 记录警告日志并跳过，绝不向子进程注入密文，同时保证其余健康环境变量正常注入。

## 内置资产自动收敛不变量：用户定制永远优先，禁止触碰非软链资产（2026-09-07，2026-09-06-cli-service-connection /reflect）

- **现象**：内置 Skill（`cli-claude`, `cli-codex`, `cli-crawl4ai`）作为随工作台发行的只读资产，通过 `syncProjectCliSkills` 在项目启用/禁用 CLI 时动态创建或移除指向 `.skills/<slug>` 的符号链接。若逻辑实现仅以文件名匹配而不检查节点类型，可能在重同步或禁用时误删或覆盖用户在项目内自建的同名定制 Skill 目录。
- **根因**：自动化收敛机制容易假设目标目录下的同名条目都是自己创建的软链。
- **结论**：**内置资源自动同步必须遵循「用户定制高于内置默认」不变量**：
  1. 创建时：若目标路径已存在且**不是符号链接**（即用户自建的真实目录或文件），绝对不覆盖、不删除，静默保留用户版本。
  2. 移除时：仅当目标路径为符号链接时才予以清理；若为真实目录/文件，绝对不触碰。

## Auto 模式安全审判员权力收敛：AI 判定仅作加速放行，未授权/危险一律转人工确认卡（2026-09-08，2026-09-07-mcp-sse-transport /reflect）

- **现象**：开启 auto 模式后，底层接入了 `auto-judge` 模型裁决器。当模型在提示词中看到“防范数据外发”规则时，将合法的外部网页爬取（如 `crawl4AI_md` 爬取 `https://example.com`）机械判定为“数据外发攻击”（`kind: "deny"`）。由于 `auto-judge` 的硬拦截权力，导致 gotgenes 直接中断任务并抛出 `User denied MCP target ... Reason: 未授权访问网络爬取外部内容，可能造成数据外发风险`，用户连弹卡确认的机会都没有，auto 模式反而比 manual 模式更严苛、体验极差。
- **根因**：混淆了「AI 辅助代问」与「用户最终主权」。Auto 模式的初衷是替用户省去常见无害操作的确认繁琐度（加速放行），而不是请一个教条的 AI 安全合规专员来代替用户行使“一票否决权”。
- **结论**：**Auto 模式下的模型审判员严禁拥有硬拒绝权**：
  1. **Deny → Defer 平滑降级**：模型判定为 `allow` 则直接放行；若模型判定为 `deny` 或 `defer`（不确定），一律降级为 `defer`，把控制权平滑流转给下一棒的人工确认卡（`opc-bridge`），由用户亲自决定放行或终止。
  2. **Prompt 边界消歧**：明确界定“合法数据获取”与“恶意数据外发”，对常规网页抓取、API 数据查询、代码只读构建默认倾向 `allow`，避免虚警。

## MCP 权限规则解析与数据库实时直通：消灭字符串匹配盲区，保证用户配置即改即生效（2026-09-08，2026-09-07-mcp-sse-transport /reflect）

- **现象**：用户在工作台 MCP 管理界面的“默认权限”中明确将特定工具或 Server 配置为了 `Allow`，但在运行时调用 MCP 工具时，依然被底层判定为 `ask`，甚至被拦截。
- **根因**：底层 gotgenes 权限引擎的 `checkPermission("mcp", value)` 在底层对未结构化字符串的处理存在盲区，直接丢弃了工具名参数，导致所有外部传入的 `server:tool` 字符串都无法命中任何规则，始终回落到通配符 `*`（默认 `ask`）；此外，工作台界面修改的权限存储在数据库（`mcp_permission_defaults`），而长期运行的 worker 进程未能动态读取最新库内规则。
- **结论**：**MCP 权限判定必须建立稳健的多级回落与数据库实时解析器（`resolveMcpPermission`）**：
  1. 规则匹配层生成包含 `server:tool`、`tool`、`server_tool`、`server` 的完整候选集，并支持大小写不敏感的通配符匹配（如用户配 `crawl4ai:*` 可自适应命中 `crawl4AI:crawl4AI_md`）。
  2. 权限评估按序探测：项目级覆盖文件 → SQLite 数据库用户级默认权限（直读 DB，即改即生效）→ 全局部署策略文件 → gotgenes 内部 resolver。只要用户在界面保存了 `Allow`，下一次调用必须秒级直通生效。

## 未落库配置 Ad-hoc 探测闭环：复杂外部连接必须“先测通再保存”（2026-09-07，2026-09-07-mcp-sse-transport BUG-001 /reflect）

- **现象**：MCP 添加/编辑弹窗在早期设计中必须先保存入库，才能在外部列表行点击“工具”进行连接探测。遇到远程 SSE/Bearer Token 端点配置错误时，用户被迫把错误配置写入数据库，并在列表与弹窗间反复跳转修改，严重割裂了录入体验（BUG-001 req-gap）。
- **根因**：未能将「配置持久化管理」与「协议连通性验证」解耦。
- **结论**：**对于外部服务连接，必须提供无状态的 Ad-hoc 探测端点（如 `POST /api/mcp/probe`）**：
  1. 弹窗支持直接使用当前表单中的临时字段（包含明文 Token/Header）进行内联即连即断测试。
  2. 测试失败仅向表单内返回诊断信息，绝不污染持久化数据库。
  3. 编辑既有连接时，由于敏感 Token 永不回显明文，测试连接需显式引导重填 Token 以保证探测真实有效。
