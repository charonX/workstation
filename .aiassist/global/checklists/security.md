...
- **Auto 模式模型裁决权力收敛（Deny → Defer）**：模型判断为 `deny` 时严禁拥有硬拒绝权，必须平滑降级为 `defer` 弹卡让人确认，避免 AI 将正常外部爬取/获取（如 crawl4ai）误判为数据外发而中断任务。
- **MCP 权限实时直通与多级解析**：用户在界面配置的工具/服务权限直接存入 SQLite DB（`mcp_permission_defaults`），运行时通过多候选解析器直读 DB 与项目覆盖配置，支持大小写不敏感与通配符匹配（如 `crawl4ai:*`），实现秒级生效。
- **Ad-hoc 无状态探测凭据安全**：支持在未保存前进行连接探测（`POST /api/mcp/probe`），探测过程仅在内存中即连即断，绝不向数据库写入未校验的草稿，探测报错中严格脱敏明文 Token。
