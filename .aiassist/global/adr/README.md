...
| ADR-042 | 文件预览面板——HTTP/SSE 数据通道（非 IPC，纯数据无 bounds 需求；复用 files/image 先例 + 既有 SSE）+ 右侧槽位互斥（预览 XOR 浏览器，实例保活）+ 图片白名单对齐附件（SVG 拒收）+ 路径识别仅行内 code | 已接受 | 2026-09-02 | 待结晶（2026-08-31-file-preview）|
