...
| conversation-space (session-domain) | `tests/capabilities/agent-dialogue/conversation-space/2026-08-16-deepen-session-domain/api/` | REQ-AGENT-112~117（2026-08-17 结晶） | `sessionDomain/sseRegistry/dependencyDirection.test.js`（待生成）：sessionDomain 纯函数收编（config 装配/投影分页/key 解析/附件规则/gitState）+ SSE 注册表 per-instance 三方法 + 依赖方向回正静态断言 + 路由瘦身 ≤650 行（ADR-030；2026-08-17 review 复核修订：~300/≤350 算术不可行，人拍板重定） |
