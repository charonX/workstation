...
| conversation-space (feishu-reset-history-archive) | `tests/capabilities/agent-dialogue/conversation-space/2026-08-19-feishu-reset-history-archive/api/` | REQ-AGENT-123~126（2026-08-19 结晶） | `feishuResetArchive.test.js`（123/124）, `feishuArchiveSessions.test.js`（125/126）, `feishuResetReceipt.test.js`（123 回执）：飞书 /reset 归档事务（旧行改名 …:gen<N> + 新活跃行）+ 异常退化分支 + 列表可见与 displayName fallback + 归档只读回看与 403 守护（ADR-037） |
