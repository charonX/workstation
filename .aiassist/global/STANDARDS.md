...
## 外部服务凭据与敏感配置规范（2026-09-03，2026-08-26-rsshub-integration /reflect）

- **凭据字典扩展与集中管理**：
  - 外部第三方服务的连接凭据（如 RSSHub/社交平台/API 服务等）统一挂载在 `settings.credentials[serviceType]` 字典下，禁止在配置根节点扁平散落命名，保持 settings 结构的通用扩展性。
- **部分更新保全密文（Patch 语义）**：
  - 更新凭据配置时，若请求体中未传递敏感字段（如 `accessKey` 为空/未传），服务层必须保留已有的加密密文，严禁直接整对象覆盖导致旧密钥被意外冲掉。
- **强脱敏与权限隔离**：
...
## Auto 模式安全仲裁：AI 仅加速放行，疑问一律降级转人工（2026-09-08，2026-09-07-mcp-sse-transport /reflect）

- **Deny → Defer 平滑降级机制**：`auto-judge` 模型裁决器的核心定位是识别安全操作并加速放行（`allow`），**严禁赋予模型对任务的硬拒绝权（硬 `deny`）**。
- **用户主权兜底**：若模型评估结果为 `deny` 或 `defer`，必须平滑降级为 `defer`，无缝流转至前端/通道的人工确认卡（`opc-bridge`），由真实用户进行最终裁决与放行。
- **Prompt 边界消歧与防过度防御**：系统提示词必须明确划定开发行为边界。对常规公网网页抓取/读取（如 crawl4ai、web_fetch）、第三方 API 查询、只读文件检查，默认倾向于 `allow`，严禁机械地将正常的网络数据获取判定为“数据外发攻击”。

## MCP 工具运行时权限多级解析与即改即生效（2026-09-08，2026-09-07-mcp-sse-transport /reflect）

- **消灭匹配盲区与候选集生成**：MCP 工具目标名称多样（`server:tool`、`tool`、`server_tool`、`server`），权限解析器（`resolveMcpPermission`）必须生成完备的候选集，并支持大小写不敏感的通配符匹配（如 `crawl4ai:*`）。
- **权限真源按序穿透**：项目级覆盖文件（`.opc/gotgenes.policy.json`） > SQLite 数据库用户级默认权限（`mcp_permission_defaults`） > 全局策略文件 > 引擎默认。
- **实时直读 DB 杜绝陈旧缓存**：用户在工作台界面保存工具默认权限后，运行时调用必须秒级感知最新库内规则，实现直通放行，严禁长常驻 worker 进程持有静态陈旧缓存。

## 外部连接 Ad-hoc 无状态探测与即测即断（2026-09-08，2026-09-07-mcp-sse-transport /reflect）

- **编辑态解耦于持久化**：外部服务（MCP Server、CLI、数据库）在表单新增或编辑时，必须提供独立的无状态探测接口（如 `POST /api/mcp/probe`），支持直接接收前端当前输入的未落库草稿（包含明文 Token/Headers）进行真实协议握手。
- **脏数据隔离与即测即断**：探测必须遵循即测即断原则（设置合理网络超时如 10s），探测结果仅向表单内返回诊断回执，探测失败严禁向持久化数据库写入半成品或错误配置。
- **凭据脱敏防回显**：编辑既有服务时，数据库中存储的密文令牌绝不下发给前端，表单通过 placeholder 占位，测试连接若需重新测试鉴权须显式提示用户输入真实 Token。
