...
> 版本：v1（2026-08-17）
> 最后更新：2026-08-17
> 来源：`prd.md` v0.2（§4 四稳定块；§10 技术方案已由 /tech-design 深潜定稿）
> 移动块：无（§5 已清空，四块全解决）
> UX 参照：N/A（纯内部架构重构，无用户界面；DESIGN/DOMAIN-MODEL 阶段跳过）
> 技术事实（tech-design 实证）：reset 清队列人拍板 A（lifecycle.remove 不碰 worker
> 侧 Map、sessionQueues 残留真实存在）；计数清时机人拍板 B（beginTurn 幂等清 +
> 取出即删，失败轮残留不混轮）；双入口 import 可行（模块零外部依赖 + toolAdapter
> 先例）；弱降级无既有断言锁定（主进程截断修复零测试冲突）。
> ADR：ADR-029（tech-design 2026-08-17 落盘：工厂模块 + 注册表统一清理 + 截断单源取强）。
...
- 优先级 P0 / 必须 / cross-module / turnEventPipeline（新增）+ worker.js + agentService.js（消费 limitSize）/ agent-dialogue / conversation-space / 单元
- 接口契约：
  - `createTurnEventPipeline({ send, log, setTimeout, clearTimeout, now })` → 实例；import 无副作用
  ...
  - 错误：无（未知 sessionKey 静默 no-op 语义在 REQ-AGENT-107 AC6）
...
6. 未知 sessionKey：`onSessionEvent("ghost-key", text_delta)` 静默 no-op——send spy 零调用、不抛异常（单元；现状语义保持）。
...
4. reset 丢排队操作（人拍板 A + E-AGENT-RESET 回执人拍板 1）：spawn + FAUX（TPS 控速造排队窗口）——prompt1 流式中 store.reset(key) → prompt2（排队中、未开始执行）收到 prompt-result `{ok:false, error:{code:"E-AGENT-RESET", reason:"会话已重置，排队中的消息已取消"}}`（resolve 非 reject——事件即结果语义）；prompt2 无任何 LLM 事件（FAUX 回声不含其文本）；不发 session-error（集成）。

## REQ-AGENT-110 256KB 截断单真源 + 主进程行为修复

- 优先级 P0 / 必须 / cross-module / turnEventPipeline（导出 limitSize）+ agentService.js（enforceSizeLimit 删除、3 调用点 248/346/963 改 import）/ agent-dialogue / conversation-space / 单元 + 集成
...
2. 事件链形状契约：text_start → text_delta×N → text_end（带 meta.durationMs）顺序与形状不变（集成）。
