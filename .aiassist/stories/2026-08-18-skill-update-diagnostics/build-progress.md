...
| REQ-022 AC1 | 更新成功 → 行内成功提示（含 slug；版本有 → 含新版本号） | `Skills.jsx` `handleUpdate` 成功后按 `freshGroups` 查该组 version：非空 → `t("skills.updateSuccessWithVersion", { slug, version })`，否则 `t("skills.updateSuccess", { slug })`；渲染绿色卡片 `data-testid="update-success"`（`--ch-success` 色变量，对称 actionError）。版本经 `useSkills.updateSource` 返回的刷新后 `groups` 取到（不读闭包旧值） | `e2e/skillUpdateDiagnostics.test.cjs` 用例 2（QA 门，toBeVisible） | COVERED |
...
- i18n：`src/renderer/i18n/zh-CN.json` / `en-US.json` skills 区补 `updateSuccess`（"已更新技能源 {{slug}}" / "Updated skill source {{slug}}"）+ `updateSuccessWithVersion`（"已更新技能源 {{slug}} → v{{version}}" / "Updated skill source {{slug}} → v{{version}}"）+ `updateLogTitle`（"更新日志" / "Update Log"）；`version` 键已有复用（组头不额外用 label，直接渲染值）。
