...
> 对应 `requirements.md` v1.1，hash `835c36c5544138cce6439e02f7ba146691088bcb08b1de2b6224f939ddbc7485`。
...
| 验收标准 | 测试方法 | 关键断言 |
|---|---|---|
| AC3：`feishuMessage` 节点视为 trigger-like，注入变量覆盖 defaultValue | 调用 `flowEngine.run()`，传入含 `feishuMessage` + agent 的 flow 与注入变量 | agent prompt 中包含注入后的 url/sender/messageId |
...
| AC2：配置面板固定展示 url/sender/messageId 且不可删除 | 选中节点，查看 properties panel | 三个变量可见；删除按钮不可见；可修改 defaultValue 并保存 |
...
原 `imRouting.test.js` 已覆盖去重、URL 路由、无绑定、draft/已删 flow 分支。本次扩展：

| 新增/变更 | 测试方法 | 关键断言 |
|---|---|---|
| AC2 增加节点存在性校验 | `createProjectFlow` 默认生成含 `feishuMessage` 节点的 flow；命中绑定后创建执行 | 执行创建成功 |
| AC4 新增：绑定 flow 缺少 `feishuMessage` 节点 | 创建无 `feishuMessage` 节点的 flow 并绑定，发送 URL 消息 | 回复配置异常提示；写「通道状态」通知；不创建执行 |
...
| 新增/变更 | 测试方法 | 关键断言 |
|---|---|---|
| AC1：链接速存模板首节点为 `feishuMessage` | instantiate 链接速存模板后查询 flow | 节点 type=`feishuMessage`，固定输出 url/sender/messageId |
| AC4：模板生成的 flow 能正确合并 IM 注入变量 | 用模板生成 flow 的 publishedNodeList 调用 `flowEngine.run()` | agent prompt 能引用注入的 url |
