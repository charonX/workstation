...
> 创建日期：2026-08-19
> 完结日期：2026-08-22（/reflect 门 2 通过；逻辑真值看代码，意图真值看 adr/ADR-037 与 CONTEXT.md）
> 最后更新：2026-08-22
...
- **留人决策结论（2026-08-22 /reflect，人拍板）**：POST alias **保留现状**——handler 内已有 `feishu:*` 403 只读守护（mode/provider 写入口各一处），`ui:*` 空间 POST=PUT 同义无害（本地桌面应用，无跨域攻击面）；行为已在 ADR-037 §13 记录，不再前移守护。
