...
|---|---|---|---|---|
| S1 | 统一输出模型基础设施：创建 `nodeRegistry.js`，改造 `upstreamVariables.js`，统一 `flowService` outputVariables 校验 | ADR-010, FLOW-032 AC5, FLOW-033 AC6, FLOW-042 AC5 | `nodeRegistry.js`, `upstreamVariables.js`, `flowService.js` | pending |
...
|---|---|---|---|
| 所有节点类型统一使用 `config.outputVariables` | `nodeRegistry.js`, `flowService.js` | `nodeRegistry.test.js`, `setVariablesUpstream.test.js` | pending |
| `upstreamVariables.js` 不再按类型 switch | `upstreamVariables.js` | `setVariablesUpstream.test.js` | pending |
| 节点类型注册表包含 `deriveOutputVariables` | `nodeRegistry.js` | `nodeRegistry.test.js` | pending |
